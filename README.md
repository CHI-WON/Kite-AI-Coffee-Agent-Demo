...
> 基于 Kite Account Abstraction SDK 的 **多 Agent 协作**支付系统

## 🎯 项目概述

本项目实现了一个"咖啡店多 Agent 系统"，展示如何使用 Kite AI 的核心能力：

| 参赛要求 | 实现方式 |
|---------|---------|
| ✅ **链上支付** | 使用 AA SDK 完成测试网 USDT 转账 |
| ✅ **Agent 身份** | **3 个独立 Agent**，各自拥有 AA 钱包 |
| ✅ **权限控制** | 多级审批流程，单笔/日限额控制 |
| ✅ **可复现性** | 完整的安装和运行说明 |

## 🤖 Multi-Agent 架构（核心亮点）

本项目采用**多 Agent 协作**架构，订单经过 3 个独立 Agent 处理：

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  ReceptionAgent │ ──▶ │  ApprovalAgent  │ ──▶ │  PaymentAgent   │
│     接待 Agent   │     │     审批 Agent   │     │     支付 Agent   │
│   (验证订单格式)  │     │   (检查权限策略)  │     │   (执行链上转账)  │
...
| Agent | 职责 | AA 钱包功能 |
|-------|------|-----------|
| **ReceptionAgent** | 接收订单、验证格式、签名确认 | 记录订单接收 |
| **ApprovalAgent** | 审核策略、检查限额、批准/拒绝 | 签名审批决策 |
| **PaymentAgent** | 执行 USDT 转账、记录交易 | 持有资金、执行转账 |

### 技术亮点

1. **多账户 AA 能力** - 3 个独立的 Account Abstraction 钱包
2. **状态机流程** - 完整的订单状态追踪
3. **职责分离** - 每个 Agent 专注单一职责
4. **可扩展架构** - 易于添加新的 Agent 角色
5. **完整审计追踪** - 每步处理都有签名记录
...
详见下方 [🌐 Web UI 模式](#-web-ui-模式前端--后端) 章节。
...
kite-ai-coffee-agent/
├── src/                          # 后端代码
│   ├── index.ts                  # CLI 入口 - 单 Agent 模式
│   ├── server.ts                 # HTTP 服务器 - 多 Agent API
│   ├── config.ts                 # 配置管理
│   │
│   ├── agents/                   # 🤖 多 Agent 系统（新增）
│   │   ├── types.ts              # 类型定义、消息格式
│   │   ├── BaseAgent.ts          # Agent 基类
│   │   ├── ReceptionAgent.ts     # 接待 Agent
│   │   ├── ApprovalAgent.ts      # 审批 Agent
│   │   ├── PaymentAgent.ts       # 支付 Agent
│   │   ├── AgentOrchestrator.ts  # 编排器（协调流程）
│   │   └── index.ts              # 导出模块
│   │
│   ├── agent/                    # 单 Agent 模式（保留）
│   │   ├── createAgent.ts
...
└── frontend/                     # 前端代码
    ...
    │       ├── OrderForm.tsx
    │       ├── OrderStatus.tsx
    │       └── AgentPipeline.tsx # 🆕 多 Agent 流程可视化
    ├── package.json
    └── vite.config.ts
```
...
│  (React + Wagmi)│   POST /order │  (Express +     │  AA SDK       │  (Kite Testnet) │
│                 │               │   CoffeeAgent)  │               │                 │
└─────────────────┘               └─────────────────┘               └─────────────────┘
       │                                  │
       │ 钱包连接                         │ Agent 决策
...
1. **ConnectWallet** - 连接钱包
   - 支持 MetaMask 等浏览器钱包
   - 自动检测网络，提示切换到 Kite Testnet

2. **OrderForm** - 下单表单
   - 选择咖啡类型（Latte / Espresso / Premium 等）
   - 输入数量
   - 显示价格（含超限提示）
   - 提交订单到后端
...
```json
{
  ...
  "status": "approved",
  "data": {
    "order": { "item": "Latte", "price": 0.03, "currency": "USDT" },
    "transactionHash": "0x...",
    "explorerUrl": "https://testnet.kitescan.ai/tx/0x..."
  }
}
```
